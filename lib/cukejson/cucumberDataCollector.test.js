...
describe("Cucumber Data Collector", () => {
  ...
  beforeEach(async () => {
    ...
    const feature = await parse(spec);
    this.testState = new CucumberDataCollector(filePath, spec, feature);
    this.testState.onStartTest();
  });

  it("runs", async () => {
    this.testState.onFinishTest();
    const json = await generateCucumberJson(this.testState);
    expect(json).to.have.length(0);
  });

  it("records pending scenarios", async () => {
    this.testState.onStartScenario(scenario, stepsToRun);
    this.testState.onFinishScenario(scenario);
    this.testState.onFinishTest();
    const json = await generateCucumberJson(this.testState);
    assertCucumberJson(json, ["skipped", "skipped", "skipped"]);
  });
  it("records passed scenarios", async () => {
    this.testState.onStartScenario(scenario, stepsToRun);
    ...
    this.testState.onFinishTest();
    const json = await generateCucumberJson(this.testState);
    assertCucumberJson(json, ["passed", "passed", "passed"]);
  });

  it("records failed scenarios", async () => {
    this.testState.onStartScenario(scenario, stepsToRun);
    this.testState.onStartStep(stepsToRun[0]);
    this.testState.onFinishStep(stepsToRun[0], statuses.PASSED);
    this.testState.onStartStep(stepsToRun[1]);
    ...
    this.testState.onFinishTest();
    const json = await generateCucumberJson(this.testState);
    assertCucumberJson(json, ["passed", "failed", "skipped"]);
  });

  it("handles missing steps", async () => {
    this.testState.onStartScenario(scenario, stepsToRun);
    this.testState.onStartStep(stepsToRun[0]);
    this.testState.onFinishStep(stepsToRun[0], statuses.PASSED);
    this.testState.onStartStep(stepsToRun[1]);
    ...
    this.testState.onFinishTest();
    const json = await generateCucumberJson(this.testState);
    assertCucumberJson(json, ["passed", "undefined", "skipped"]);
  });
});
